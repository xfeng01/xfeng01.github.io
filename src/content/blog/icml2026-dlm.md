...
| 3 | [**Breaking the Factorization Barrier in Diffusion Language Models**](https://arxiv.org/abs/2603.00045)<br>*Ian Li, Zilei Shao, Benjie Wang, Rose Yu, Guy Van den Broeck, Anji Liu* | poster | **今年被低估最严重的一篇。** 直指并行生成的根本限制——同时预测多 token 时隐含的条件独立假设。不是「再加个 planner」，而是用 tractable probabilistic modeling 正面处理 factorization barrier。很多 decoding trick 都在这个 barrier 之下打转 |
